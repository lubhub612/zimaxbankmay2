...
export default function MultiplyArea() {
  ...
  const viewStakeZIMAX = async () => {
    ...
    // let lastIndex=myString0.lastIndexOf(",");
    // let myString1=myString0.slice(0,lastIndex+1);
            ...
            {unlock && (
              <UnLockArea>
                ...
                {  stakingData.map((item) => (
                  ...
                  <li> {item.unlock_time}</li>
                  <li> {item.claimed}</li>
               <li> <button disabled={ item.unlock_time > timestamp  ?  true : false} >{t("UNLOCK ")}</button> </li>
                </ul>
     ))}
